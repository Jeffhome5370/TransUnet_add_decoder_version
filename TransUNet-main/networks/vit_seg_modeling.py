...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if config.patches.get("grid") is not None:   # ResNet
            ...
            patch_size = (img_size[0] // 16 // grid_size[0], img_size[1] // 16 // grid_size[1]) #(512//16//16, 512//16//16) = (2, 2)
            patch_size_real = (patch_size[0] * 16, patch_size[1] * 16) #(32, 32)
            n_patches = (img_size[0] // patch_size_real[0]) * (img_size[1] // patch_size_real[1]) #(512//32)*(512//32) = 16*16 = 256 
            self.hybrid = True
        else:
    ...
    def forward(self, x):
        if self.hybrid:
            x, features = self.hybrid_model(x) #x: Resnet50最後一層的輸出、features: 中間層的特徵列表 
        else:
            features = None
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.ffn(x)
        x = x + h
        return x, weights #輸出跑完1個Block的Transformer encoder
...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        if self.config.n_skip != 0:
            skip_channels = self.config.skip_channels #skip_channels = config.skip_channels = [512, 256, 64, 16]
...
# -------------------------------------------------------------------------
#  Transformer Decoder for Mask Classification (Decoder-only / Hybrid)
# -------------------------------------------------------------------------
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1):
        ...
        # 1. Masked Cross-Attention (Cross-Attn First)
    ...
    def forward(self, query, key_value, current_mask=None):
        ...
        # query: (B, Q, C)
        # key_value: (B, L, C) -> CNN Features (Flattened)
        
        # 準備 Mask (Bias)
        attn_bias = None
        if current_mask is not None:
            # current_mask shape: (B, Q, L) -> 需要擴展為 (B*num_heads, Q, L)
            # 這是 PyTorch MultiheadAttention 對 3D mask 的要求
            B, Q, L = current_mask.shape
            
            # 建立基礎 Mask (全 0)
            attn_bias = torch.zeros_like(current_mask)
            
            # 填入極小負值 (Masking)
            attn_bias.masked_fill_(current_mask < 0.5, -1e9) # 或 float('-inf')
...
class TransUNet_TransformerDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        encoded_feats, _ = self.transformer.encoder(x, hidden_states)  # (B, L, C)
